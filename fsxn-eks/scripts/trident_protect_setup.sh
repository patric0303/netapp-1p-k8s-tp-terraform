...
AWS_ACCESS_KEY_ID           = jsondecode(file(var.aws_cred_file)).aws_access_key_id
AWS_SECRET_ACCESS_KEY       = jsondecode(file(var.aws_cred_file)).aws_secret_access_key
...
install_trident_protect() {
	...
	helm install trident-protect netapp-trident-protect/trident-protect --set clusterName=${aws_cluster_name} --version ${aws_trident_protect_version} --create-namespace --namespace trident-protect
}

create_appvault_cr() {
        echo "    --> creating appVault secret"
        kubectl -n trident-protect create secret generic ${aws_s3_bucket} --from-literal=accessKeyID=${aws_access_key_id} --from-literal=secretAccessKey=${aws_secret_access_key}
